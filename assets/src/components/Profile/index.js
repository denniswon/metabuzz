    ...
    <div>
      ...
      <div className="profile-details-wrapper">
        ...
        {account && account.username === userParam ? null : (
          <span onClick={() => startChat()} className="new-msg">
          <ICON_NEWMSG />
          </span>
        )}
        <div
          ...
          account && account.following.includes(user._id)
            ? 'unfollow-switch profile-edit-button'
            : 'profile-edit-button'
          }
        >
          {account && account.username === userParam ? (
        ...
        <div className="profile-bio">{user.description}</div>
        <div className="profile-info-box">
          {user.location.length > 0 && <ICON_MARKDOWN />}
          <div
          className={
            user.location.length > 0 ? 'profile-location' : ''
          }
          >
          {' '}
          {user.location}{' '}
